...
const Transactions = () => {
  ...
  return (
    <div className="p-6 pt-2 min-h-screen">
      ...
      {/* ---------- 📜 Table Section ---------- */}
      {activeTab === "bookings" && (
        <div className="background-gradients border border-gray-700 rounded-xl p-6 mb-6">
          <div className="w-full rounded-xl overflow-x-auto space-y-4">
            {/* Table Header */}
            <div className="grid grid-cols-7 gap-4 text-left text-sm font-semibold text-gray-300 border-b border-gray-700 pb-4">
              <div>#</div>
              <div>Lister Name</div>
              <div>User Name</div>
              <div>Total Price</div>
            ...
            {displayedTxns.length === 0
              ...
              : displayedTxns.map((txn, idx) => (
                  ...
                  >
                    ...
                    <div>{txn.lister.name}</div>
                    <div>{txn.user.name}</div>
                    <div>${txn.totalPrice}</div>
                    <div>${txn.platformFee}</div>
                    <div>
                      {txn.adminCommission}% (${txn.adminCommissionAmount})
                    </div>
                    <div>{moment(txn.createdAt).format("DD, MMM YYYY")}</div>
                    {/* <div>
                      <button
                        onClick={() => handleViewClick(txn._id)}
